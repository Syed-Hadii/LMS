...
export const createChartAccount = async (req, res) => {
  try {
    ...
    // Create a main account if no sub-account data is provided
    if (!subCat || subCat.length === 0) {
      ...
      // Destructure sub-account data from the request
      const {
        parent_id,
        name: subAccountName,
      ...
      // Add the new sub-account to the main account's subCat array
      mainAccount.subCat.push(newSubAccount);

      // Update the main account's total credit or debit amount
...
export const deleteChartAccount = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error deleting chart account", error });
  }
};
