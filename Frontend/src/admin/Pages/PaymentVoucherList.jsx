...
const PaymentVoucherList = () => {
  ...
  const fetchVouchers = async () => {
    ...
    try {
      const response = await axios.get(
        `${url}/paymentreciept/get?page=${currentPage}&limit=${recordsPerPage}&search=${searchTerm}`
      );
           if (response) {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await axios.put(
        `${url}/paymentreciept/update/${editingVoucherId}`,
        editableVoucher
      );
  ...
  const deleteVoucher = async (voucherId) => {
    setLoading(true);
    try {
      const response = await axios.delete(`${url}/paymentreciept/delete`, {
        data: { voucherId },
      });
      if (response.data.success) {
         toast.dismiss();
 ...
 useEffect(() => {
   const filterVouchers = () => {
     ...
     // Ensure vouchers is an array before filtering
   ...
   if (vouchers) filterVouchers(); // Only run if vouchers is defined
 }, [searchTerm, vouchers]);
...
useEffect(() => {
  ...
  return (
    <div className="p-6 mx-auto">
      <h1 className="text-xl mb-5 font-semibold text-left">Payment Vouchers</h1>
